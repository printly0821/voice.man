"""
KoBERT Model Wrapper
SPEC-NLP-KOBERT-001 TAG-001: KoBERT model wrapper with GPU/CPU auto-detection
"""
...
class KoBERTModel:
    """
    KoBERT model wrapper with automatic GPU/CPU detection
    ...
    _instance: Optional["KoBERTModel"] = None
    _lock = Lock()
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info(f"KoBERT model loaded successfully on {self.device.value}")

        except Exception as e:
            logger.error(f"Error loading KoBERT model: {e}")
            raise
        finally:
            self._loading = False
